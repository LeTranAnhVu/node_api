...
import { extractJobIdFromString } from '../../features/background-jobs/background-job-event'
...
const processJob = async (job: any): Promise<void> => {
    if (job.name === ProductQueue.jobs.importCSV) {
        ...
        if (originalName.endsWith('.csv')) {
            const bckgroundJobId = extractJobIdFromString(job.id)
            await productService.bulkCreateFromFile(path, bckgroundJobId)
        } else {
